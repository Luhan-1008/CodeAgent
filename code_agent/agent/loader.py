...
from code_agent.config.config import Config
from code_agent.utils.constants import DEFAULT_AGENT_FILE, DEFAULT_TAG
...
class AgentLoader:
    ...
    def build(cls,
              ...
              **kwargs) -> Agent:
        # 入口职责：把“路径配置 + 运行时覆盖配置”合并成最终 agent_config，
        # 再根据 type/code_file 构造具体 Agent 实例。
        ...
        if config is not None:
            # CLI 传入 config 时，优先把它合并到基础配置上，便于“模板 + 覆盖”。
        ...
        if agent_config is not None:
            # type 决定走内置哪种 Agent；code_file 则表示走外部自定义 Agent。
