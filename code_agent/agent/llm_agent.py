...
from code_agent.callbacks import Callback, callbacks_mapping
from code_agent.llm.llm import LLM
from code_agent.llm.utils import Message
from code_agent.memory import Memory, memory_mapping
from code_agent.memory.mem0ai import Mem0Memory, SharedMemoryManager
from code_agent.rag.base import RAG
from code_agent.rag.utils import rag_mapping
from code_agent.tools import ToolManager
from code_agent.utils import async_retry, read_history, save_history
from code_agent.utils.constants import (DEFAULT_OUTPUT_DIR, DEFAULT_TAG,
                                        DEFAULT_USER)
...
class LLMAgent(Agent):
    ...
    async def step(
        ...
        # load_cache 分支用于“故障重试/续跑”：如果最后一条已经是 assistant，
        # 这轮不再重新问模型，避免产生与历史不一致的新回答。
    ...
    async def run_loop(self, messages: Union[List[Message], str],
        ...
        try:
            ...
            # 1) 初始化可扩展能力：回调、模型、运行时、工具、记忆、RAG。
            # 这一段是“开机阶段”，成功后主循环才会启动。
            ...
            # 2) 主循环：每轮由 step() 执行“LLM生成 -> 工具调用 -> 回调 -> 持久化”。
